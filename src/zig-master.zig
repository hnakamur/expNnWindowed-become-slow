...
fn expNnWindowed(
    ...
    try powers[1].copy(x_abs.toConst());
    var i: usize = 2;
    ...
    while (i < 1 << n) : (i += 2) {
        ...
        var p1 = &powers[i + 1];
        try p.sqr(p2);
        try zz.divTrunc(&r, p, m_abs);
        p.swap(&r);
        try p1.mul(p, x_abs);
        try zz.divTrunc(&r, p1, m_abs);
        p1.swap(&r);
    }
    ...
    while (i >= 0) : (i -= 1) {
        ...
        while (j < @bitSizeOf(Limb)) : (j += n) {
            ...
            try zz.mul(&z, &powers[yi >> (@bitSizeOf(Limb) - n)]);
            zz.swap(&z);
            try zz.divTrunc(&r, &z, m_abs);
            z.swap(&r);
...
test "expNnWindowed" {
    ...
    const f = struct {
        fn f(want: Managed, x: *const Managed, y: *const Managed, m: *const Managed) !void {
